...
//
    ...
    protected BinaryNode add(String key, BinaryNode node) {
        ...
        if (key.compareTo(node.key) == 0) {
            // replace the key in this node with a new key
//            node.key = key;
        	node.count+=1;
        	
            // alternative code creates new node, leaves old node unchanged:
            //return new BinaryNode<T>(key, node.left, node.right);
        }
        else {
...
public WordFrequency[] toArray() {
	...
	WordFrequency[] WFArray = new WordFrequency[numNodes];
	TreeIterator treeIt = new TreeIterator(root);
for(int i =0; i<numNodes; i++) {
	String key = treeIt.inorderNext();
	long count =get(key);
	WFArray[i] =  new WordFrequency(key.toString(), count);
}
	return WFArray;
